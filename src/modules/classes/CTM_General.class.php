...
class CTM_General extends CTM_MSSQL
{
	...
	public function WebSite_Information($Return)
	{
		...
		switch($controller->getLicenseInfo("license", "licenseType"))
		{
			case "trial_expire" : $expire = true; $Type = "Demonstrativa (Com validade)"; break;
			case "trial_noexpire" : $expire = false; $Type = "Demonstrativa"; break;
			case "complete_expire" : $expire = true; $Type = "Completa (Com validade)"; break;
			case "complete_noexpire" : $expire = false; $Type = "Completa"; break;
		}
		...
		if($expire == true)
		{
			$Key = $Not[0].date("d/m/Y", $controller->getLicenseInfo("license", "expireTime")).$Close;
		}
		else
		{
		...
		if($Return == 2)
		{
			return $controller->getLicenseInfo("license", "serial");
		}
		if($Return == 3)
		{
			return $Type;
		...
		if($Return == 4)
		{
			return $Not[1].$controller->version->getVersion("full").$Close;
		}
		if($Return == 5)
		{
			return $controller->getLicenseInfo("owner", "name");
		}
	}
	/**
	 *	1.7.16 : Get Hex Void
